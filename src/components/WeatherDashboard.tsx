...
import { Cloud, CloudLightning, CloudRain, CloudSnow, Sun, Wind, AlertCircle, Thermometer, MapPin, RefreshCw, Navigation, Shield, Settings, X } from 'lucide-react';
...
export function WeatherDashboard() {
  ...
  const getWeatherIcon = (condition: string) => {
    ...
    if (c.includes('thunder') || c.includes('storm')) return <CloudLightning className="w-8 h-8 text-amber-400" />;
    if (c.includes('rain') || c.includes('shower')) return <CloudRain className="w-8 h-8 text-blue-400" />;
    if (c.includes('snow') || c.includes('ice')) return <CloudSnow className="w-8 h-8 text-white" />;
